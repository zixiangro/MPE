...
class Model(nn.Module):
    def __init__(self, return_feature=False, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.feature_channel = last_channel
...
class PointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor", use_norm=False,
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
        ...
        self.embedding = ConvBNReLU1D(3+3*use_norm, embed_dim, bias=bias, activation=activation)
        assert len(pre_blocks) == len(k_neighbors) == len(reducers) == len(pos_blocks) == len(dim_expansion), \
            "Please check stage number consistent for pre_blocks, pos_blocks k_neighbors, reducers."
        self.local_grouper_list = nn.ModuleList()
        self.pre_blocks_list = nn.ModuleList()
    ...
    def forward(self, x, cls_label, norm_plt=None):
        xyz = x.permute(0, 2, 1)
        if self.use_norm:
...
def pointMLP(num_classes=40, return_feature=False, **kwargs) -> Model:
    return Model(return_feature=return_feature, points=1024, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                   k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2], **kwargs)


def pointMLPElite(num_classes=40, return_feature=False, **kwargs) -> Model:
    return Model(return_feature=return_feature, points=1024, class_num=num_classes, embed_dim=32, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLP_partseg(num_classes=50, return_feature=False, use_norm=False, **kwargs) -> PointMLP:
    if return_feature:
        model = Model(return_feature=True, points=2048, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
                    activation="relu", bias=True, use_xyz=True, normalize="anchor",
                    dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                    k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4], **kwargs)
    else:
        model = PointMLP(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                    activation="relu", bias=True, use_xyz=True, normalize="anchor", use_norm=use_norm,
                    dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                    k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
