...
def geometric_point_descriptor(x, k=3, idx=None):
    ...
    neighbors = neighbors.permute(0, 3, 1, 2).contiguous()  # B,C,N,k
...
class CAA_Module(nn.Module):
    ...
    def forward(self,x):
        ...
        x_hat = x.permute(0, 2, 1).contiguous()
        proj_query = self.query_conv(x_hat) 
        proj_key = self.key_conv(x_hat).permute(0, 2, 1).contiguous() 
...
class GBNet(nn.Module):
    def __init__(self, args, output_channels=40, return_feature=False):
    ...
    def forward(self, x):
        ...
        if self.return_feature: return global_max
...
def get_model(output_channels=40, return_feature=False):
    ...
    return GBNet(args=args(), output_channels=output_channels, return_feature=return_feature)