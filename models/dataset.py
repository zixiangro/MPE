...
DATA_DIR = '/home/zxluo/platform/datas'
...
def load_color_partseg():
    colors = []
    labels = []
    f = open(os.path.join(DATA_DIR, 'meta/partseg_colors.txt'))
    ...
    for row in range(0, img_size):
        ...
        for column in range(0, img_size):
            ...
            if color_index >= 50:
                cv2.imwrite(os.path.join(DATA_DIR, 'meta/partseg_colors.png'), img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
def load_color_semseg():
    colors = []
    labels = []
    f = open(os.path.join(DATA_DIR, 'meta/semseg_colors.txt'))
    for line in json.load(f):
        colors.append(line['color'])
        labels.append(line['label'])
    ...
    for _ in range(0, img_size):
        ...
        for _ in range(0, img_size):
            ...
            label_index = label_index + 1
            if color_index >= 13:
                cv2.imwrite(os.path.join(DATA_DIR, 'meta/semseg_colors.png'), img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                return np.array(colors)
...
class ShapeNet(Dataset):
    def __init__(self, num_points, partition='train', whole=False):
        self.data_root = os.path.join(DATA_DIR, 'ShapeNet55-34')
        self.pc_path = os.path.join(DATA_DIR, 'ShapeNet55-34/pointcloud')
        self.npoints = 8192
        
        self.data_list_file = os.path.join(self.data_root, f'{partition}.txt')
        
        self.sample_points_num = num_points

        with open(self.data_list_file, 'r') as f:
            lines = f.readlines()
        if whole and partition == 'train':
        ...
        with open(os.path.join(DATA_DIR, 'ShapeNet55-34/shapenet_synset_dict.json'), 'r') as f:
            tax2label = json.load(f)
            self.tax2label = tax2label
    ...
    def __getitem__(self, idx):
        ...
        # data = rotate_pointcloud(data)

        return data, (sample['taxonomy_id'], sample['model_id'])
        # return data, self.tax2label[sample['taxonomy_id']]
...
class ScanObjectNN(Dataset):
    def __init__(self, num_points, partition='train', rotate=False):
        self.rotate = rotate
        self.partition = partition    
        self.root = os.path.join(DATA_DIR, 'ScanObjectNN/main_split')
        if partition == 'train':
        ...
        data = np.array(h5['data']).astype(np.float32)
        idx_pts = np.arange(data.shape[1])
        np.random.shuffle(idx_pts)
        self.data = data[:,idx_pts[:num_points],:]
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train':
            pointcloud = translate_pointcloud(pointcloud)
            if self.rotate and np.random.randn() > 0:
